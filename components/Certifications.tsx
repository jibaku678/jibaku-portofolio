...
export default function Certifications() {
  ...
  const nextFireSlide = () => {
    setFireDocIdx((prev) => (prev === fireImages.length - 1 ? 0 : prev + 1))
  }

  const prevFireSlide = () => {
    setFireDocIdx((prev) => (prev === 0 ? fireImages.length - 1 : prev - 1))
  }
  ...
  return (
    <section id="certifications" className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-slate-200 dark:border-slate-800/60">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {trainingList.map((item, idx) => (
          <motion.div
            ...
            onClick={() => item.isInteractive && setIsFireModalOpen(true)}
      ...
      {/* Lightbox Modal: Fire Safety Certificate & Field Documentation Slider */}
      <AnimatePresence>
        {isFireModalOpen && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setIsFireFullScreen(!isFireFullScreen)}
                    className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-amber-500 transition-colors cursor-pointer flex items-center gap-1.5 text-xs font-bold"
                  >
                  ...
                  <button
                    onClick={() => { setIsFireModalOpen(false); setIsFireFullScreen(false); }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-4 flex-1 overflow-y-auto">
                ...
                {/* Sisi Kanan: Slider Foto Dokumentasi Praktik */}
                <div className="relative flex flex-col items-center justify-center bg-slate-100 dark:bg-slate-950 rounded-xl p-3 border border-slate-200 dark:border-slate-800">
                  ...
                  <div className="relative w-full aspect-video rounded-lg overflow-hidden bg-slate-900 flex items-center justify-center shadow-md group">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); prevFireSlide(); }}
                      className="absolute left-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-slate-950/70 text-white hover:bg-slate-950 transition-all cursor-pointer"
                    >
                      <ChevronLeft className="w-4 h-4" />
                    </button>
                    <button
                      onClick={(e) => { e.stopPropagation(); nextFireSlide(); }}
                      className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-slate-950/70 text-white hover:bg-slate-950 transition-all cursor-pointer"
                    >
                  ...
                  <div className="flex items-center gap-1.5 mt-3">
                    {fireImages.map((_, dIdx) => (
                      <button
                        key={dIdx}
                        onClick={(e) => { e.stopPropagation(); setFireDocIdx(dIdx); }}
                        className={`h-1.5 rounded-full transition-all cursor-pointer ${
                          fireDocIdx === dIdx ? 'w-5 bg-amber-500' : 'w-1.5 bg-slate-300 dark:bg-slate-700'
                        }`}
                      />
                    ))}
                  </div>
