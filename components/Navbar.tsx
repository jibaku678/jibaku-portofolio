  ...
  return (
    <motion.header 
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 px-4 sm:px-6 lg:px-8 py-4 ${
        scrolled 
          ? 'bg-white/85 dark:bg-slate-950/85 backdrop-blur-2xl border-b border-slate-200/80 dark:border-slate-800/80 shadow-sm dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)] py-3' 
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <motion.a 
          ...
          className={`text-xs sm:text-sm font-black tracking-wider uppercase flex items-center gap-2.5 px-4 py-2 rounded-xl backdrop-blur-md group transition-all duration-500 ${
            scrolled 
              ? 'bg-slate-100/80 dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800/80 text-slate-900 dark:text-white shadow-xs' 
              : 'bg-white/70 dark:bg-slate-900/50 border border-slate-200/60 dark:border-white/10 text-slate-900 dark:text-white shadow-sm'
          }`}
        >
        ...
        <nav className={`hidden md:flex items-center gap-1 px-3.5 py-1.5 rounded-full backdrop-blur-xl transition-all duration-500 ${
          scrolled 
            ? 'bg-slate-100/80 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/90 shadow-xs' 
            : 'bg-white/70 dark:bg-slate-900/50 border border-slate-200/60 dark:border-white/10 shadow-sm'
        }`}>
          {navLinks.map((link, idx) => (
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className="px-3.5 py-1 text-xs font-bold text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 uppercase tracking-widest transition-colors rounded-full hover:bg-emerald-500/10"
            >
              {link.name}
            </motion.a>
          ))}
        ...
        <div className="hidden md:flex items-center gap-2.5">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-white transition-all cursor-pointer shadow-xs text-xs font-bold uppercase tracking-wider backdrop-blur-md"
        ...
        <div className="flex items-center gap-2 md:hidden">
          <button
            onClick={toggleDarkMode}
            className="flex items-center gap-1 px-3 py-2 rounded-xl bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 text-[10px] font-mono uppercase font-bold shadow-xs backdrop-blur-md"
            aria-label="Toggle Theme"
          >
            {isDarkMode ? <Sun className="w-3.5 h-3.5 text-amber-400" /> : <Moon className="w-3.5 h-3.5 text-slate-700" />}
          ...
          <button
            ...
            className="p-2.5 rounded-xl bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 transition-colors shadow-xs backdrop-blur-md"
          >
